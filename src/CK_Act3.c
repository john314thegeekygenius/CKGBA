...
void SpawnEater(Sint16 x, Sint16 y)
{
	...
	ck_newobj->ydir = 1;
	NewState(ck_newobj, &s_eaterstand1, CKS_TEATER);
}
...
void T_EaterTeleport(objtype *ob)
{
	...
	for(int i = player->uuid; i < CK_NumOfObjects; i++){
		...
		if (ob2->obclass == bonusobj)
		{
			...
			ob->y = ob2->y;
			NewState(ob, &s_eatertport5, CKS_TESMOKE);
			return;
		}
	}
	RemoveObj(ob);
	...
	for (y=ob->tiletop; y<=ob->tilebottom; y++, map+=rowdiff)
	{
		for (x=ob->tileleft; x<=ob->tileright; x++, map++)
		{
			...
			if (intile == INTILE_DROP || intile >= INTILE_BONUS100 && intile <= INTILE_AMMO)
			{
				...
				CK_SetSprite(&ck_newobj->sprite, CKS_PVAPER); // TODO: Hmmm
...
void SpawnMimrock(Sint16 x, Sint16 y)
{
	...
	NewState(ck_newobj, &s_mimrock, CKS_MIMROCK);
}
...
void SpawnDopefish(Sint16 x, Sint16 y)
{
	...
	ck_newobj->ydir = 1;
	NewState(ck_newobj, &s_dopefish1, CKS_DOPEFISH);
}
...
void T_Burp(objtype *ob)
{
	...
	ck_newobj->yspeed = -20;
	ck_newobj->xspeed = 4;
	NewState(ck_newobj, &s_bubble1, CKS_LBUBBLE);
...
void SpawnSchoolfish(Sint16 x, Sint16 y)
{
	...
	ck_newobj->y = CONVERT_TILE_TO_GLOBAL(y);
	ck_newobj->ydir = ck_newobj->xdir = 1;
	NewState(ck_newobj, &s_schoolfish1, CKS_SCHOOLFISH);
}
...
void SpawnPixie(Sint16 x, Sint16 y)
{
	...
	ck_newobj->ydir = ck_newobj->xdir = 1;
	NewState(ck_newobj, &s_pixie, CKS_SPRITE);
}
...
void T_PixieShoot(objtype *ob)
{
	...
	NewState(ck_newobj, &s_pixiefire1, CKS_EBLAST);
...
void SpawnMine(Sint16 x, Sint16 y, Sint16 dir)
{
	...
	NewState(ck_newobj, &s_mine, CKS_MINE);
}
...
void SpawnLindsey(Sint16 x, Sint16 y)
{
	...
	ck_newobj->ydir = 1;
	NewState(ck_newobj, &s_lindsey1, CKS_PLINDSEY);
}
...
void T_DartShoot(objtype *ob)
{
	...
	case 0:
		ck_newobj->xdir = 0;
		ck_newobj->ydir = -1;
		NewState(ck_newobj, &s_dartup1, CKS_DART);
		break;
	case 1:
		ck_newobj->xdir = 1;
		ck_newobj->ydir = 0;
		NewState(ck_newobj, &s_dart1, CKS_DART);
		break;
	case 2:
		ck_newobj->xdir = 0;
		ck_newobj->ydir = 1;
		NewState(ck_newobj, &s_dartdown1, CKS_DART);
		break;
	case 3:
		ck_newobj->xdir = -1;
		ck_newobj->ydir = 0;
...
void SpawnScuba(Sint16 x, Sint16 y)
{
	...
	NewState(ck_newobj, &s_scuba, CKS_SWIMSUIT);
}
